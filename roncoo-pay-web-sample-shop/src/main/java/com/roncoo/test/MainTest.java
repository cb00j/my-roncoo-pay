...
public class MainTest {
    ...
    public static void main(String[] args) {
        ...
        List<MerchantKey> listMerchantKey = new ArrayList<>();//添加商户key
        ...
        for (MerchantKey merchantKey : listMerchantKey) {
            ...
            new Thread(() -> testGateWayPay(payKey, paySecret)
            ).start();
        }
    ...
    private static void testGateWayPay(String payKey, String paySecret) {
        ...
        for (int i = 0; i < 100000; i++) { //每条线程生成数
            try {
            ...
            System.out.println("请求网关创建订单:" + resultStr);
            ...
            String notifyResultStr = SimpleHttpUtils.httpGet("http://192.168.0.133:8080/roncoo-pay-web-gateway/scanPayNotify/notify/TEST_PAY_HTTP_CLIENT", notifyMap);
            System.out.println("扣款结果:" + notifyResultStr);
        }
    }
    ...
    private static Map<String, Object> getInitRequestMap(String merchantOrderNo, String payKey, String paySecret) {
        ...
        paramMap.put("returnUrl", "http://192.168.0.133:8080/roncoo-pay-web-sample-shop/roncooPayNotify/notify"); // 页面通知返回url
        paramMap.put("notifyUrl", "http://192.168.0.133:8080/roncoo-pay-web-sample-shop/roncooPayNotify/notify");// 后台消息通知Url
